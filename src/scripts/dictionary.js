...
//   let data = await fetchWordData(searchWord); 

//   // returns semi IPA
//   // return data[0]?.hwi?.prs[0]?.mw.split('-'); 

//   // returns the syllables
//   return data[0].hwi.hw.split("*"); 
// }
...
export const syllableToIpa = (syllable) => {

  // from phoneme-synthesis library - https://github.com/itinerarium/phoneme-synthesis
  const MAPPINGS = [
    { 'src': /^\s*\//g, 'dest': '' },
    { 'src': /\/\s*$/g, 'dest': '' },
  ...
  for (let i = 0; i < MAPPINGS.length; i++) {
    syllable = syllable.replace(MAPPINGS[i].src, MAPPINGS[i].dest);
    console.log(`[${MAPPINGS[i].src}] translates to ${uipa}`);
  }
